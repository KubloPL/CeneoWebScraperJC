...
def extract():
    if request.method == 'POST':
        ...
        if response.status_code == requests.codes['ok']:
            ...
            if opinions_count:
                ...
                stats = {
                    ...
                    'recommendations_distribution': opinions.recommendation.value_counts().reindex(['Polecam', 'Nie polecam', 'Brak rekomendacji'], fill_value=0).to_dict()
                }
        ...
        return render_template('extract.jinja', error=error)
...
def product(product_id):
    ...
    return render_template('product.html.jinja', product_id=product_id, opinions=opinions_df.to_html(classes='table table-striped', index=False))
